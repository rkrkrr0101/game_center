...
class MemberControllerE2ETest @Autowired constructor(
    ...
    fun 멤버를_저장할수_있다(){
        ...
        //w,t
        mvc.perform (MockMvcRequestBuilders.post("""/member/save""")
    ...
    fun 멤버를_업데이트할수_있다(){
        ...
        //w,t
        mvc.perform (MockMvcRequestBuilders.put("""/member/${member.id}""")
            .contentType("application/json")
            .content(reqBodyJson)
        )
            .andExpect(status().isOk)
    ...
    fun 멤버를_업데이트할때_해당되는_id의_멤버가_없으면_httpcode400이_발생한다(){
        ...
        //w,t
        mvc.perform (MockMvcRequestBuilders.put("""/member/${100}""")
            .contentType("application/json")
            .content(reqBodyJson)
        )
            .andExpect(status().isBadRequest)
    ...
    fun 멤버를_삭제할수_있다(){
        ...
        memberRepository.save(member)
        //w,t
        mvc.perform (MockMvcRequestBuilders.delete("""/member/${member.id}""")
        )
            .andExpect(status().isOk)
    }
    @Test
    fun 해당되는_id의_멤버가_없으면_삭제시_httpcode400이_발생한다(){
        mvc.perform (MockMvcRequestBuilders.delete("""/member/${100}""")
        )
            .andExpect(status().isBadRequest)
    }
}
